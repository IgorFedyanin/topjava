...
public abstract class JdbcMealRepository<T> implements MealRepository {
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        T sdt = toDBDateTime(startDateTime);
        T edt = toDBDateTime(endDateTime);
        return jdbcTemplate.query(
                "SELECT * FROM meals WHERE user_id=?  AND date_time >=  ? AND date_time < ? ORDER BY date_time DESC",
                ROW_MAPPER, userId, sdt, edt);
    }
    ...
    static class HSQLDBJdbcMealRepository extends JdbcMealRepository<Timestamp>{
